...
private:
    ...
    void dissociateObservedSeq() {
        ...
        for (int i = 1; i < observedSeq.size(); i++) {
            ...
            Network obsFormationNet (obsFormationSt, isDirected);
            Network obsDissolutionNet (obsDissolutionSt, isDirected);
    ...
    void sampler(int num_exchangeMCiter) {
        ...
        //exchange sample
        STERGMnet1TimeSampler_1EdgeMCMC exSeqGenerator(param_newFormation, param_newDissolution, observedSeq[lag_startTimePoint]);
...
public:
    ...
    BSTERGM_MCMC_RandomLag(Col<double> initialParam_formation, Col<double> initialParam_dissolution, vector<Network> observedSeq) {
        ...
        isDirected = observedSeq[0].is_directed_graph();
...
//                    (double)exchange_FormationSeq[t].get_undirected_k_starDist(2) - exchange_combinedSeq[t - 1].get_undirected_k_starDist(2)
//            };
//            model_delta_exchangeDissolution_2 += {
//                (double)exchange_DissolutionSeq[t].get_n_Edge() - exchange_combinedSeq[t - 1].get_n_Edge(),
//                    (double)exchange_DissolutionSeq[t].get_undirected_k_starDist(2) - exchange_combinedSeq[t - 1].get_undirected_k_starDist(2)
//            };
//        }
//        for (int t = 1; t < T_time; t++) {
//            model_delta_obsFormation_3 += {
//                (double)observed_FormationSeq[t].get_n_Edge() - observedSeq[t - 1].get_n_Edge(),
//                    (double)observed_FormationSeq[t].get_undirected_k_starDist(2) - observedSeq[t - 1].get_undirected_k_starDist(2)
//            };
//            model_delta_obsDissolution_4 += {
//                (double)observed_DissolutionSeq[t].get_n_Edge() - observedSeq[t - 1].get_n_Edge(),
//                    (double)observed_DissolutionSeq[t].get_undirected_k_starDist(2) - observedSeq[t - 1].get_undirected_k_starDist(2)
//            };
//        }
//
//        log_r_val += dot(param_lastFormation - param_newFormation, model_delta_exchangeFormation_1 - model_delta_obsFormation_3);
